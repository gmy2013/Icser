...
def create_json_with_focuses(src_sentences, summaries, attn_indices, output_file):
    output = []

    for src, summary, attn in zip(src_sentences, summaries, attn_indices):
        focus_tokens = [src[i] for i in attn]
        new_src = src + focus_tokens

        output.append({
            "source": src,
            "summary": summary,
            "source_with_focus": new_src,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        max_attention_indices = attn.max(dim=-1)[1]
        return output, attn, coverage, max_attention_indices
...
class TransformerDecoder(DecoderBase):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layer):
            ...
            max_attention_indices_list.append(max_attention_indices)
        ...
        attns["max_attention_indices"] = max_attention_indices_list
